...
def _init_dir(_dir_name):
    ...
    Args:
      _dir_name: Path string to the folder we want to create.
    """
...
def ensure_file_exists(_file_path):
    """Ensures file exists or creates new file
        Args:
            _file_path
    """
...
class FilesAndPathUtils(object):
...
class CnnFileUtils(FilesAndPathUtils):
    """Utility class for network files management"""

    def __init__(self, parent_cnn_dir):
        super(CnnFileUtils, self).__init__(parent_cnn_dir)
    ...
    def test_file(self, _file_path):
        """Joins test directory path to passed file path
          Returns:
            joined test directory and data file path
        """
        return self.join_path(self.test_dir, _file_path)
